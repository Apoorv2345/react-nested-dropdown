...
export const SubDDItems= ({subItemsHeading, subItems, selectSubItem, checkboxes, clearSubItemList})=> {
    return <div className="subItems-cnt">
        ...
        <ul className="subItems">
            {subItems.map(({id, label})=> {
                return <SubItem className={'item'} id={id} key={id} title={label} selectItem={()=>{selectSubItem(id)}} itemChbk={checkboxes}/>;
            })}
        </ul>
    </div>;
}
